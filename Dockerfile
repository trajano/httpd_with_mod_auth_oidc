...
RUN apt-get -qq update \
 ...
 && curl -sL https://github.com/zmartzone/mod_auth_openidc/archive/v$MOD_OPENIDC_VERSION.tar.gz > src.tar.gz \
 && tar zxf src.tar.gz \
...
RUN apt-get -qq update \
 && apt-get -qq install --no-install-recommends -y ca-certificates libcjose0 libhiredis0.14 \
 && rm -rf /var/lib/apt/lists/* \
 && echo Include /usr/local/apache2/conf/extras/mod_auth_openidc.conf >> /usr/local/apache2/conf/httpd.conf
